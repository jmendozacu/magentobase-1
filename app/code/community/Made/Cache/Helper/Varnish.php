...
class Made_Cache_Helper_Varnish extends Mage_Core_Helper_Abstract
{
    ...
    public function getRequestTtl($request)
    {
        ...
        if (!$this->_matchRoutesAgainstRequest($cacheRoutes, $request)
            ...
            //Mage::log('Check2: ' .$this->_matchRoutesAgainstRequest($cacheRoutes, $request));
            //Mage::log('Check222: ' .$this->_matchRoutesAgainstRequest('madecache/varnish/cookie', $request));
            return null;
        }

        Mage::log('Check3' .Mage::getStoreConfig('cache/varnish/ttl'));
